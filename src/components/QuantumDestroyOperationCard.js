...
export default class QuantumDestroyOperationCard extends Card {
  ...
  getHTML() {
    return `<div class="card gate destroy" draggable="true" ondragstart="drag(event)"><div class="card-title">Gate</div><img src="${this.cardData.gateIconPath}" class="card-symbol" /><div class="card-description">${this.cardData.gateType}</div></div>`
  }
}